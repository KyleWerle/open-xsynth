...
// NOTE: kept at exactly 16 bytes (4 x uint32) for the checksum in CopyInputs.
struct Inputs {
  // Sub-cell touch position per axis: 0..250 (signal-weighted centroid),
  // 255 on no touch.
  uint8_t touch[2];

  // Total capacitive coupling across detected keys -> pressure (0..254).
  uint8_t pressure;

  // Padding (keeps the struct word-aligned / 16 bytes).
  uint8_t reserved0;
  ...
  // Padding to 16 bytes (4 x uint32).
  uint8_t reserved1[2];
};
...
static void InitTouch() {
  ...
  // Disable AKS (Adjacent Key Suppression) so neighbouring keys can detect
  // together. This lets ReadTouch() compute a centroid for sub-cell position
  // and removes the between-pad "stick" that AKS hysteresis caused.
...
// Reads a 16-bit little-endian touch-controller register from one bus.
// (QT2120 sequential read returns LSB at the register addr, then MSB.)
static uint16_t ReadTouchReg16(uint8_t reg, uint16_t data_pin) {
  uint8_t b[2];
  b[0] = reg;
  touch_i2c.Send(0x1c, b, 1);
  touch_i2c.Recv(0x1c, 2);
  touch_i2c.GetResponse(data_pin, b, 2);
  return (uint16_t)((b[1] << 8) | b[0]);
}

// Phase 2b: for each axis, read the per-key Key Signal (0x34) and Reference
// (0x4C) of the detected keys, weight a centroid by |signal - reference|
// (continuous sub-cell position, 0..250, 255 = no touch), and sum the deltas
// into a pressure value (total capacitive coupling). AKS is disabled in
// InitTouch so neighbouring keys co-detect.
static void ReadTouch() {
  uint8_t buf[2];
  static uint16_t data_pins[2] = {GPIO_PIN_3, GPIO_PIN_4};

  // Detection status (reg 3), both busses in parallel.
  buf[0] = 3;
  touch_i2c.Send(0x1c, buf, 1);
  touch_i2c.Recv(0x1c, 2);

  uint16_t keymask[2] = {0, 0};
  for (int ch=0; ch<2; ++ch) {
    if (touch_i2c.Acknowledged(data_pins[ch])) {
      touch_i2c.GetResponse(data_pins[ch], buf, 2);
      keymask[ch] = buf[0] | (buf[1] << 8);
    }
  }
  ...
  for (int ch=0; ch<2; ++ch) {
    uint16_t keys = keymask[ch];
    uint32_t wsum = 0;  // sum(key * delta)
    uint32_t dsum = 0;  // sum(delta)
    for (int key=0; key<11; ++key) {
      if (keys & (1 << key)) {
        uint16_t sig = ReadTouchReg16(0x34 + key * 2, data_pins[ch]);
        uint16_t ref = ReadTouchReg16(0x4c + key * 2, data_pins[ch]);
        int delta = (int)sig - (int)ref;
        if (delta < 0) { delta = -delta; }       // magnitude of coupling
        wsum += (uint32_t)key * (uint32_t)delta;
        dsum += (uint32_t)delta;
      }
    }
    if (dsum > 0) {
      // Signal-weighted centroid -> 0..250, inverted to screen co-ordinates.
      current_inputs.touch[ch] = (uint8_t)(250 - (wsum * 25) / dsum);
      total_delta += dsum;
    } else {
      current_inputs.touch[ch] = 0xff;
    }
  }

  // Pressure: total coupling across both axes, scaled into 0..254 (tunable).
  uint32_t p = total_delta >> 2;
  if (p > 254) { p = 254; }
  current_inputs.pressure = (uint8_t)p;
}
...
static void CopyInputs() {
  static_assert(
      sizeof(current_inputs) == sizeof(uint32_t) * 4,
      "Hard coded copy does not match source size.");
  static_assert(
      sizeof(i2c_message.inputs) == sizeof(uint32_t) * 4,
      "Hard coded copy does not match destination size.");

  // This is hard coded as 4 uint32_t copies, which ties in with the checksum
  // calculation.
